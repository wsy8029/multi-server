...
GlobalServer_CM* pGlobalServer = NULL;
...
BOOL onConsoleEvent(DWORD event) {
	...
	case CTRL_CLOSE_EVENT:
		if (pGlobalServer)
		{
			pGlobalServer->m_bLoopStop = TRUE;
...
int __cdecl main(int argc, char** argv)
{
	...
	info.pszAppName = L"GlobalServer"; // App name
	info.pszEmailSubject = L"GlobalServer 1.0.0 Error Report"; // Email subject
	...
	pGlobalServer = new GlobalServer_CM();
	if(!pGlobalServer->Initialize(svrID))
	{
		Sleep(5000);
		pGlobalServer->Release();
		delete pGlobalServer;
		return -1;
	}
	...
	while(1)
	{
		...
		pGlobalServer->Update();

		if(pGlobalServer->m_bLoopStop)		// 종료 처리!
		{
			break;
		}
	}
